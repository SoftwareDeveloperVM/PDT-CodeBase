...
class FabTnPol(object):
    ...
    # epg = name of the EPG
    # encap = VLAN encapsulation value
    # deploy = lazy | immediate|
    # mode= regular | trunk|
    # status= created | created,modified | deleted

    def aep_epg(self, **kwargs):
        required_args = {'name': '',
                         'tenant': '',
                         'ap_name': '',
                         'epg': '',
    ...
    # Method must be called with the following kwargs.
    # vip = ip addr for vip (must be /32)
    # scope = scope of address private | shared | public
    # nhaddr = ip addr of next hop
    def epg_static_route(self, **kwargs):
        ...
        template_file = "epg_static_route.json"
        template = self.templateEnv.get_template(template_file)

        payload = template.render(templateVars)

        uri = 'mo/uni/tn-{}'.format(templateVars['tenant'])
        status = post(self.apic, payload, self.cookies, uri, template_file)
        return status
