...
def pod_policies(apic, cookies, wb, wr_wb):
    ...
    podpol = acipdt52.FabPodPol(apic, cookies)
...
def access_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    accpol = acipdt52.FabAccPol(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def tn_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    tnpol = acipdt52.FabTnPol(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def l3_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    l3pol = acipdt52.FabL3Pol(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def vmm_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    vmm = acipdt52.FabVMM(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def fab_admin_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    fabadmin = acipdt52.FabAdminMgmt(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def mpod_policies(apic, cookies, wb, wr_wb):
    ...
    rows = ws.nrows
    func_list = findKeys(ws, rows)
    mpod = acipdt52.Mpod(apic, cookies)
    stdout_log(wr_ws.name, None)
    for func in func_list:
        count = countKeys(ws, rows, func)
        var_dict = findVars(ws, rows, func, count)
...
def take_snapshot(apic, cookies, snapshot_name):
    query = acipdt52.Query(apic, cookies)
    ...
    cfgmgmt = acipdt52.FabCfgMgmt(apic, cookies)
...
def main():
    ...
    fablogin = acipdt52.FabLogin(apic, user, pword)
